...
/** This package's own name: the only thing an update is ever allowed to install. */
const PLUGIN_PACKAGE = 'dsh-plugin-poker';

/**
 * Where this plugin was installed from, i.e. the directory to run a package manager in.
 *
 * The plugin lives at `<root>/node_modules/dsh-plugin-poker/lib`, so the root is the
 * nearest ancestor that actually contains this package under its `node_modules`. Walking
 * up instead of guessing means a globally installed plugin, a profile install and a
 * linked checkout all resolve to the right place.
 * @returns an absolute path, or the package's parent when nothing matches.
 */
function findInstallRoot() {
  let dir = HERE;
  for (let level = 0; level < 8; level += 1) {
    const candidate = path.join(dir, 'node_modules', PLUGIN_PACKAGE);
    try {
...
/** The lock files present in a directory, which is how the package manager is chosen. */
function lockFilesIn(dir) {
  const names = ['pnpm-lock.yaml', 'yarn.lock', 'package-lock.json'];
  try {
...
/**
 * Run a package manager, capturing its output.
 *
 * argv form, no shell, and a hard timeout: an update button must never be able to hang the
 * host or to be talked into running something else by whatever the page (or a release tag)
 * happens to contain.
 * @param options - `{ command, args, cwd, timeoutMs }`.
 * @returns `{ code, stdout, stderr }`.
 */
function runInstall(options) {
  return new Promise((resolve) => {
    const timeoutMs = Math.max(5000, Math.min(300000, Number(options.timeoutMs) || 120000));
    let child;
    try {
      child = spawn(options.command, options.args, { cwd: options.cwd, shell: false, windowsHide: true });
    } catch (error) {
...
export async function handlePokerRequest(req, res) {
  try {
    ...
    if (op === 'update-apply') {
      // The click that actually upgrades. Everything that runs is decided HERE: the
      // package, the version and the directory come from this installation, never from the
      // request body - the page can only ask for "the newest release of this repository".
      if (typeof m.applyUpdate !== 'function') {
        sendJson(res, 503, { ok: false, error: '\u81ea\u52a8\u66f4\u65b0\u9700\u8981\u91cd\u65b0\u52a0\u8f7d\uff08\u91cd\u542f\u4e00\u6b21 dsh web\uff09' });
        return;
      }
      const settings = settingsOf(m);
      const installRoot = findInstallRoot();
      // A checkout updates by pulling; an installed package by its package manager. The
      // package is not on npm, so getting this wrong means a 404 instead of an update.
      const gitRoot = [installRoot, path.resolve(HERE, '..')].find((dir) => {
        try {
      ...
      const result = await m.applyUpdate({
        current: currentVersion(),
        repo: settings.updateRepo,
        packageName: PLUGIN_PACKAGE,
        installRoot,
        gitRoot,
        manager: m.pickPackageManager(lockFilesIn(installRoot)),
        run: runInstall,
      });
      sendJson(res, 200, { ok: true, result });
      return;

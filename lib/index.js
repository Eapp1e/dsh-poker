 ...
 */
const IMPL_FILES = ['impl.js', 'ai.js', 'coach.js', 'engine.js', 'bots.js', 'cards.js', 'evaluator.js', 'render.js'];
 ...
 */
let memorySettings = { enabled: false, seats: 1 };
...
export async function handlePokerRequest(req, res) {
  try {
    ...
    const m = await impl();
    if (op === 'settings') {
      // The plugin's own settings surface. GET reports the current values (the API
      // key is never echoed back - only whether one is stored), POST patches them.
      if (!hasAiSurface(m)) {
      ...
      if ((req.method ?? 'GET').toUpperCase() === 'POST') {
        const allowed = {};
        for (const key of m.AI_FIELDS) if (body && Object.prototype.hasOwnProperty.call(body, key)) allowed[key] = body[key];
        const saved = await m.writeSettings(hostCtx, allowed, memorySettings);
        memorySettings = saved;
        await retuneAiSeats(saved);
        sendJson(res, 200, { ok: true, settings: publicSettings(saved) });
        return;
      }
      sendJson(res, 200, { ok: true, settings: publicSettings(settingsOf(m)) });
      return;
    }
    if (op === 'new') {
      ...
      const state = saveTable(m.opNew(body, { paused: true, aiSeats: aiSeatCount(m) }));
      sendJson(res, 200, {
        ok: true,
...
/**
 * Rebuild the round trip the browser gets: the settings minus the secret, plus a
 * plain flag saying whether a key is stored (so the form can say "已保存" without
 * ever receiving it back).
...
function publicSettings(settings) {
  return {
    ...
    hasApiKey: typeof settings.apiKey === 'string' && settings.apiKey !== '',
  };
}

/**
 * Point the live table's AI seats at the configuration just saved.
 *
 * The seats the AI plays are the first N opponents, so raising or lowering the count
 * takes effect immediately instead of needing a new table.
 * @param settings - the configuration just saved.
 */
async function retuneAiSeats(settings) {
  const table = currentTable();
  if (!table) return;
  const seats = settings.enabled === true ? settings.seats : 0;
...
/**
 * The settings view, tolerant of an implementation that predates the AI surface.
 *
 * A hot reload can pair this entry with a staged `impl.js` that does not export the
 * settings helpers yet, and a missing export must never take the tools down: the
 * defaults simply apply (AI off, pure heuristics).
 * @param m - the implementation module.
 * @returns a complete settings object.
 */
function settingsOf(m) {
  if (!m || typeof m.readSettings !== 'function') return { enabled: false, seats: 0 };
  try {
    return m.readSettings(hostCtx, memorySettings);
  } catch {
    return { enabled: false, seats: 0 };
  }
}
...
function aiSeatCount(m) {
  ...
  return settings.enabled === true ? Math.max(0, Number(settings.seats) || 0) : 0;
}
  ...
  (async () => {
    ...
    try {
      while (guard < 60) {
        ...
        const settings = settingsOf(m);
        if (settings.enabled !== true) break;
...
export function apply(ctx) {
  ...
  register({
    ...
    execute: async (args) => {
      const m = await impl();
      const started = saveTable(m.opNew(args, { aiSeats: aiSeatCount(m) }));

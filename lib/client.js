...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.dshp-setLabel{flex:0 0 116px;display:inline-flex;align-items:center;gap:4px;color:var(--dsw-alias-label-secondary,inherit)}',
      ...
      '.dshp-setFoot{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:14px}',
      // What saving means sits beside the button that saves, not in a paragraph up top.
      '.dshp-setSaveNote{flex:0 1 auto;min-width:0}',
      '.dshp-setUpdateState{font-weight:600}',
      '.dshp-setUpdateNew{color:var(--dsw-alias-state-warn-primary,#ffd166)}',
      '.dshp-setLinkBtn{cursor:pointer;border:0;background:0 0;padding:0;font:inherit;font-size:12px;color:var(--dsw-alias-label-tertiary,#8a8a8a);text-decoration:underline}',
      '.dshp-setLinkBtn:hover{color:var(--dsw-alias-label-primary,inherit)}',
      '.dshp-setLinkBtn:disabled{opacity:.5;cursor:default}',
      ...
      '.dshp-updateDot{width:6px;height:6px;border-radius:50%;background:var(--dsw-alias-state-warn-primary,#ffd166);flex:none;box-shadow:0 0 0 2px var(--dsw-alias-bg-base,rgba(0,0,0,.25))}',
    ...
    /**
     * The release check, shared by the sidebar entry and the settings page.
     *
     * Checked ONCE per page load, on the entry that is always mounted: "自动检查更新" has to
     * happen without the player opening the settings, and the host caches the answer for ten
     * minutes, so this is one GitHub call per window rather than one per render.
     */
    const UPDATE_STATE = { checked: false, running: false, result: null, applied: null, dismissed: false };
    const UPDATE_LISTENERS = new Set();
    /** "Not now": hide the prompt for the rest of the page's life, without any state to keep. */
    function dismissUpdate() {
      UPDATE_STATE.dismissed = true;
      notifyUpdate();
    }
    /** POST JSON and parse the answer. The one network shape this bundle uses. */
    ...
    function notifyUpdate() {
      for (const listener of UPDATE_LISTENERS) {
        try {
          listener();
        } catch (error) {
          // A surface that has gone away must not stop the others from re-rendering.
        }
      }
    }
    async function checkUpdate(force) {
      if (UPDATE_STATE.running) return UPDATE_STATE.result;
      UPDATE_STATE.running = true;
      notifyUpdate();
      try {
        const answer = await postJson('/poker/update-check', force === true ? { force: true } : {});
        UPDATE_STATE.result = answer && answer.ok
          ? answer.result
          : { ok: false, behind: false, lines: [(answer && answer.error) || '\u68c0\u67e5\u66f4\u65b0\u5931\u8d25'] };
      } catch (error) {
        UPDATE_STATE.result = { ok: false, behind: false, lines: ['\u68c0\u67e5\u66f4\u65b0\u5931\u8d25\uff08\u5bbf\u4e3b\u9762\u677f\u8fde\u4e0d\u4e0a\uff09'] };
      } finally {
    ...
    async function applyUpdate() {
      if (UPDATE_STATE.running) return UPDATE_STATE.applied;
      UPDATE_STATE.running = true;
      UPDATE_STATE.applied = null;
      notifyUpdate();
      try {
        const answer = await postJson('/poker/update-apply', {});
        UPDATE_STATE.applied = answer && answer.ok
          ? answer.result
          : { ok: false, lines: [(answer && answer.error) || '\u66f4\u65b0\u5931\u8d25'] };
      } catch (error) {
        UPDATE_STATE.applied = { ok: false, lines: ['\u66f4\u65b0\u5931\u8d25\uff08\u5bbf\u4e3b\u9762\u677f\u8fde\u4e0d\u4e0a\uff09'] };
      } finally {
        UPDATE_STATE.running = false;
        notifyUpdate();
      }
      return UPDATE_STATE.applied;
    }
    /** Subscribe to the release check; the settings page and the entry both do. */
    function useUpdateState() {
      const [, setTick] = React.useState(0);
      React.useEffect(() => {
        const bump = () => setTick((value) => value + 1);
        UPDATE_LISTENERS.add(bump);
        return () => UPDATE_LISTENERS.delete(bump);
      }, []);
      return UPDATE_STATE;
    }
    ...
    function HeaderAction(props) {
      ...
      // The automatic half of "自动检查更新": one lookup per page load, on the surface that
      // is always mounted, so a new release is known before the settings are ever opened.
      const update = useUpdateState();
      React.useEffect(() => {
        if (hidden) return undefined;
        if (!UPDATE_STATE.checked && !UPDATE_STATE.running) void checkUpdate(false);
        return undefined;
      }, [hidden]);
          ...
          props.wide === false
            ...
            : React.createElement(
                ...
                [
                  ...
                  // A new release is worth one small dot on the entry, not a popup: the
                  // settings page is where the numbers and the update button live.
                  update.result && update.result.behind === true && update.dismissed !== true
                    ? React.createElement('span', {
                        className: 'dshp-updateDot',
                        key: 'update',
                        title: `\u6709\u65b0\u7248\u672c ${update.result.latest}\uff08\u5f53\u524d v${update.result.current}\uff09\u2014\u2014\u8bbe\u7f6e\u2192\u5fb7\u5dde\u6251\u514b \u91cc\u53ef\u4ee5\u4e00\u952e\u66f4\u65b0`,
                      })
                    : null,
                ],
              ),
    ...
    function PokerSettings() {
      ...
      // Results of the `action` fields (the release check, the connection probe), by field.
      const [actionResults, setActionResults] = React.useState({});
      // The shared release state: the automatic check may have answered before this page was
      // ever opened, in which case there is nothing to press. If it has not run yet (no
      // sidebar entry on screen), this page runs it, so the answer is always here.
      const update = useUpdateState();
      React.useEffect(() => {
        if (!UPDATE_STATE.checked && !UPDATE_STATE.running) void checkUpdate(false);
        return undefined;
      ...
      /**
       * Run a descriptor's action and keep its answer.
       *
       * Two answer shapes come back, both printed by the same cell: the release check and the
       * update reply with `lines` to print verbatim, and the connection probe with a timing.
       * The probe lives here too because it is a field in the AI group now - it belongs with
       * the configuration it tests, not in the footer next to 保存.
       */
      const runAction = async (field, confirmFirst) => {
        // A destructive action asks first. The update runs a package manager against this
        // installation, so it is one click plus one confirmation, never a single stray
        // click - and re-checking (nothing newer) asks nothing, because it changes nothing.
        if (confirmFirst === true && field.confirm && typeof globalThis.confirm === 'function' && !globalThis.confirm(field.confirm)) return;
        ...
        try {
          ...
          // Whether it updated or just re-checked, the shared release state is now stale:
          // re-read it so the version line and the entry's dot agree with reality.
          if (field.action === 'update-apply' && answer && answer.ok) {
            if (answer.result && answer.result.updated === true) setStatus(`\u5df2\u66f4\u65b0\u5230 v${answer.result.version}`);
            void checkUpdate(true);
            if (answer.result && answer.result.updated === true) void loadPluginSettings(true);
          }
        } catch (error) {
          setActionResults((current) => Object.assign({}, current, {
      ...
      const control = (field) => {
        ...
        if (field.kind === 'action') {
          const result = actionResults[field.key] || null;
          const busy = update.running || (result && result.running === true);
          // The lines a result prints: the update and the release check send `lines`; the
          // connection probe answers with a timing instead.
          const lines = result && Array.isArray(result.lines)
            ? result.lines
            : result && (result.ms !== undefined || result.error !== undefined)
              ? [result.ok
                  ? `\u2705 \u8fde\u901a\uff1a${result.ms}ms \u00b7 ${result.model || ''}${result.reply ? ` \u00b7 \u56de\u590d\u201c${result.reply}\u201d` : ''}`
                  : `\u274c \u5931\u8d25\uff1a${result.error || '\u539f\u56e0\u4e0d\u660e'}`]
              : null;
          // Only the UPDATE field carries the release state. Every other action (the
          // connection probe) is a plain button that prints whatever it got back - giving
          // them all the update UI put a second "更新到 v9.9.9" on the AI group.
          if (field.action !== 'update-apply') {
            return React.createElement('span', { className: 'dshp-setActionCell' },
              React.createElement('button', {
                ...
                disabled: busy,
                title: field.hint,
                onClick: () => runAction(field, false),
              }, busy ? '\u6d4b\u8bd5\u4e2d\u2026' : field.label),
              lines
          ...
          // Status first, action second - the shape a market page uses, and the honest one:
          // "already the newest" is a FACT to print, not a button to press. The button only
          // appears when pressing it would change something.
          const behind = Boolean(update.result && update.result.behind === true);
          const state = busy ? '\u68c0\u67e5\u4e2d\u2026'
            : behind && update.result && update.result.latest
              ? `\u6709\u65b0\u7248\u672c ${update.result.latest}\uff08\u5f53\u524d v${update.result.current}\uff09`
              : update.checked && update.result && update.result.ok
                ? `\u5df2\u662f\u6700\u65b0\u7248\u672c\uff08v${update.result.current}\uff09`
                : update.checked
                  ? '\u6682\u65f6\u67e5\u4e0d\u5230\u6700\u65b0\u7248\u672c'
                  : '\u6b63\u5728\u68c0\u67e5\u2026';
          const cells = [
            React.createElement('span', {
              className: 'dshp-setValue dshp-setUpdateState' + (behind ? ' dshp-setUpdateNew' : ''),
              key: 'state',
              title: update.result && Array.isArray(update.result.lines) ? update.result.lines.join(' ') : undefined,
            }, state),
          ];
          if (behind) {
            cells.push(React.createElement('button', {
              className: 'dshp-btn dshp-btnPrimary dshp-setTestBtn',
              type: 'button',
              key: 'update',
              disabled: busy,
              title: field.hint,
              onClick: () => runAction(field, true),
            }, busy ? '\u66f4\u65b0\u4e2d\u2026' : `\u66f4\u65b0\u5230 ${update.result.latest}`));
            cells.push(React.createElement('button', {
              className: 'dshp-setLinkBtn',
              type: 'button',
              key: 'later',
              title: '\u9690\u85cf\u63d0\u793a\uff08\u4e0b\u6b21\u5237\u65b0\u9875\u9762\u4f1a\u518d\u63d0\uff09',
              onClick: () => dismissUpdate(),
            }, '\u672c\u6b21\u5ffd\u7565'));
          } else {
            // Still reachable without a button: a quiet link that bypasses the 10-minute cache.
            cells.push(React.createElement('button', {
              className: 'dshp-setLinkBtn',
              type: 'button',
              key: 'recheck',
            ...
            if (update.result && update.result.url) {
              cells.push(React.createElement('a', {
                className: 'dshp-setLink',
                key: 'page',
      ...
      // A row is a label plus a control. The explanation lives in the tooltip instead of
      // under every row: a settings page that prints all of its own documentation is a wall
      // of text, and the one line a reader needs is exactly the one they hover.
      const row = (field) => React.createElement('label', {
        className: 'dshp-setRow',
        key: field.key,
      ...
      const rendered = groups.map((group) => React.createElement('div', { className: 'dshp-setGroup', key: group.id },
        React.createElement('h4', { className: 'dshp-setGroupTitle', title: group.hint || undefined }, group.label),
      ...
      return React.createElement('div', { className: 'dshp-set' },
        ...
        rendered,
        React.createElement('div', { className: 'dshp-setFoot' },
          React.createElement('button', {
          ...
          // What saving MEANS belongs next to the button that saves, not in a paragraph
          // under the title where it explains nothing the reader is looking at.
          React.createElement('span', {
            className: 'dshp-setHint dshp-setSaveNote',
            title: '\u9762\u677f\u7ea7\u7684\u5f00\u5173\uff08\u56de\u653e\u8282\u594f\u7b49\uff09\u5237\u65b0\u9875\u9762\u540e\u751f\u6548',
          }, '\u4fdd\u5b58\u540e\u7acb\u5373\u751f\u6548'),
    ...
    exports.__testing = {
      loadPluginSettings,
      usePluginSettings,
      // The release state is module-level (one check per page load), so a test that wants to
      // observe the first check has to start from "never checked".
      resetUpdate: () => {
        UPDATE_STATE.checked = false;
        UPDATE_STATE.running = false;
        UPDATE_STATE.result = null;
        UPDATE_STATE.applied = null;
      },
    };

 ...
 * @param style - its EFFECTIVE personality (declared dials, adjusted to the table).
 * @param legal - its legal action menu.
 * @param declared - the raw personality, used for the talk pool only.
 * @returns `{ action, amount?, talk? }`.
 */
function decidePreflop(state, player, style, legal, declared = style) {
  ...
  if (!raised) {
    if (percentile >= 1 - openRange && legal.canRaise) {
      ...
      return {
        ...
        talk: talkFrom(state, TALK.bet, declared),
      };
    }
    ...
    return { action: 'fold', talk: talkFrom(state, TALK.fold, declared) };
  }

  // Continuing against a raise is priced by the PRICE and by position. The seat's
  // own opening range is the ceiling - a hand not worth opening is not worth calling
  // a raise with - but the width comes from how many players still act behind: the
  // big blind closes the action and can defend very wide, the first seat cannot.
  //
  // Two bugs lived here. The old numbers used 1.5x the OPENING range, so everyone
  // continued against a raise (76% against a big one, 62% against a stack-committing
  // one) - "an all-in gets called by half the table". The first tightening overshot
  // and cut the flop rate from 59% to 41%, which is the other half of the same
  // complaint. Price and position, not one multiplier for both.
  ...
  const positionalCeiling = behind <= 1 ? 0.55 : behind === 2 ? 0.42 : behind === 3 ? 0.32 : 0.24;
  const continueRange = stackCommitted
    ? Math.min(0.1, openRange * 0.2)
    : raiseInBlinds <= 3.5
      ? Math.min(positionalCeiling, openRange)
      : raiseInBlinds <= 7
        ? Math.min(positionalCeiling * 0.6, openRange * 0.6)
        : Math.min(positionalCeiling * 0.4, openRange * 0.4);
  ...
  if (percentile >= 1 - continueRange) return { action: 'call', talk: talkFrom(state, TALK.call, declared) };
  if (potOdds < 0.2 && percentile >= 0.55) return { action: 'call' };
  return { action: 'fold', talk: talkFrom(state, TALK.fold, declared) };
}
...
/**
 * What the table is actually doing with its chips, measured over the hands played.
 *
 * `tableRead` above sees personalities; this sees behaviour. The difference matters
 * because the dials are only a starting point: a declared rock who keeps calling is
 * a station, and a table that folds to every bet is worth bluffing no matter what
 * its profiles say.
 * @param state - game state.
 * @param seat - the seat doing the reading.
 * @returns counts and rates over the opponents with a usable sample.
 */
export function observedRead(state, seat) {
  const read = { sample: 0, loose: 0, aggressive: 0, foldy: 0, vpip: 0, pfr: 0, foldToBet: 0 };
  for (const other of state.players) {
    if (other.seat === seat || other.out) continue;
    const tally = other.observed;
    if (!tally || tally.hands < 6) continue;
    read.sample += 1;
    const vpip = tally.vpip / tally.hands;
    const pfr = tally.pfr / tally.hands;
    const foldToBet = tally.facedBet >= 4 ? tally.foldedToBet / tally.facedBet : 0.5;
    read.vpip += vpip;
    read.pfr += pfr;
    read.foldToBet += foldToBet;
    if (vpip > 0.5) read.loose += 1;
    if (pfr > 0.3) read.aggressive += 1;
    if (tally.facedBet >= 4 && foldToBet > 0.65) read.foldy += 1;
  }
  if (read.sample > 0) {
    read.vpip /= read.sample;
    read.pfr /= read.sample;
    read.foldToBet /= read.sample;
  }
  return read;
}

/**
 * The personality the seat is playing RIGHT NOW: its declared dials, moved by what
 * the table has been doing.
 *
 * Every shift is small and bounded, so the six personalities stay recognisable -
 * this is an adjustment, not a replacement. The rules, in poker terms:
 *
 * - a table that puts money in loosely gets less action from everyone (tighten, and
 *   bluff less, because bluffs need folds);
 * - a table that folds to pressure gets bluffed and stolen from more;
 * - a table that raises a lot gets respected (tighten up and stop paying off).
 * @param state - game state.
 * @param seat - the seat being adjusted.
 * @param style - its declared personality.
 * @returns an effective `{ tightness, aggression, bluff, trap }`.
 */
export function adjustStyle(state, seat, style) {
  const read = observedRead(state, seat);
  const clamp = (value) => Math.max(0.02, Math.min(0.98, value));
  if (read.sample === 0) return { ...style };
  const loose = read.loose / read.sample;
  const aggressive = read.aggressive / read.sample;
  const foldy = read.foldy / read.sample;
  // Pressure from a loose table lands harder on a hand that was already giving
  // action (a station should not become a nit the moment it meets another one).
  const tightShift = loose * 0.06 + aggressive * 0.05;
  const bluffFactor = (1 - loose * 0.3) * (1 + foldy * 0.7);
  const aggressionShift = foldy * 0.05 - loose * 0.04;
  return {
    tightness: clamp(style.tightness + tightShift),
    aggression: clamp(style.aggression + aggressionShift),
    bluff: clamp(style.bluff * bluffFactor),
    trap: style.trap,
    // Kept for the talk pool and for anything that wants to know the baseline.
    label: style.label,
    read,
  };
}
...
export function decideBotAction(state, seat) {
  ...
  const declared = BOT_STYLES[player.style] ?? BOT_STYLES.pro;
  // The dials actually in play: the declared personality moved by what the table has
  // been doing (see `adjustStyle`). Everything below reads this, not the raw style,
  // which is what makes the opponents' rates drift with the game.
  const style = adjustStyle(state, seat, declared);
  ...
  if (state.street === 'preflop') return decidePreflop(state, player, style, legal, declared);
  const pot = potTotal(state);
  ...
  // Behaviour read as well as profile read: a station that keeps calling is still a
  // station, and a table that folds to pressure is worth bluffing.
  const observed = style.read ?? observedRead(state, seat);
  if (observed.sample > 0) {
    read.stations += observed.loose;
    if (observed.foldToBet > 0.6) read.tight += 1;
  }
  ...
  if (toCall === 0) {
    if (commit && legal.canAllIn) {
      return { action: 'allin', talk: talkFrom(state, TALK.bet, declared) };
    }
    ...
    if (wantRaise || bluffing) {
      ...
      return {
        action: 'raise',
        amount: target,
        talk: talkFrom(state, bluffing && !wantRaise ? TALK.bluff : TALK.bet, declared),
      };
    }
    return { action: 'check' };
  }
  ...
  if (effective > raiseThreshold && legal.canRaise && roll(state) > style.trap
    ...
    return { action: 'raise', amount: target, talk: talkFrom(state, TALK.bet, declared) };
  }
  ...
  if (bluffRaise && legal.canRaise && stackPressure < 0.35) {
    ...
    return { action: 'raise', amount: target, talk: talkFrom(state, TALK.bluff, declared) };
  }
  ...
  if (worthThePrice || cheapCall || bluffCatch) {
    ...
    return { action: 'call', talk: talkFrom(state, TALK.call, declared) };
  }
  return { action: 'fold', talk: talkFrom(state, TALK.fold, declared) };
}

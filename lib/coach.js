...
import { BOT_STYLES, decideBotAction, estimateEquity, playsTheBoard, preflopPercentile, preflopStrength } from './bots.js';
import { legalActions, potTotal } from './engine.js';
import { gtoCoaching, priceSizes } from './gto.js';
 ...
 */
/**
 * How often a raise takes the pot down BEFORE the flop.
 *
 * The tally splits bets faced into all-streets and postflop, so the preflop half is the
 * difference. Preflop is where most folds live, and using the whole number for a flop bet
 * (or the postflop number for a raise) reads the table wrong in opposite directions.
 * @param state - game state.
 * @param seat - the hero's seat.
 * @param street - the current street.
 * @returns a fold frequency in 0.05..0.95.
 */
export function preflopFoldRate(state, seat, street) {
  if (street !== 'preflop') return observedFoldRate(state, seat);
  const others = state.players.filter((player) => player.seat !== seat && !player.out);
  let faced = 0;
  let folded = 0;
  for (const player of others) {
    const tally = player.observed;
    if (!tally) continue;
    faced += (tally.facedBet ?? 0) - (tally.facedBetPost ?? 0);
    folded += (tally.foldedToBet ?? 0) - (tally.foldedToBetPost ?? 0);
  }
  if (faced < 6) return 0.5;
  return Math.max(0.05, Math.min(0.95, folded / faced));
}

/**
 * Map an engine action onto the LINE the GTO layer groups by.
 * @param action - `fold` / `check` / `call` / `bet` / `raise` / `allin`.
 * @returns the line name.
 */
function lineOfAction(action) {
  const kind = String(action || '').toLowerCase();
  if (kind === 'bet' || kind === 'raise' || kind === 'allin') return 'bet';
...
export function coachFor(state, seat, options = {}) {
  ...
  // The GTO layer replaces the rule-of-thumb plan with an EV comparison over the range
  // the opponents' actions imply, and adds its own two sections. Same spot, same
  // numbers, different reasoning - which is the whole point of offering both.
  let gto = null;
  if (state.phase === 'betting' && options.mode === 'gto') {
    // The range plan's own answer, used as the tie-break between EV-equivalent lines
    // preflop. It is the tuned policy, so the GTO headline and the opponents' logic agree
    // about what a hand is worth - the EV table then explains WHY.
    const preferredLine = legal ? lineOfAction(decideBotAction(state, seat).action) : null;
    // Everybody who could call is already all-in: there is no bet to make and no raise to
    // make it with, only a call or a fold. Pricing a raise anyway gave it a 45% chance of
    // "taking the pot down" against a player who cannot fold, which is how the coach came
    // to recommend a 208BB shove over somebody's shove.
    const callersAllIn = opponents.length > 0 && opponents.every((player) => player.allIn === true);
    const foldEquity = callersAllIn ? 0 : takeDownProbability(preflopFoldRate(state, seat, street), opponents.length);
    // Raising over a REAL raise gets folds less often (the raiser has shown strength); calling
    // over the blind does not - a big blind's 1BB is a blind, not a bet, and halving the fold
    // equity there made every 2.5BB open look like a loser.
    const facingRealRaise = Number(state.currentBet || 0) > Number(bigBlind || 0);
    gto = gtoCoaching({
      state,
      seat,
      legal,
      ...
      stack: hero.stack,
      foldEquity,
      raiseFoldEquity: callersAllIn ? 0 : foldEquity * (facingRealRaise ? 0.5 : 1),
      sizes: callersAllIn ? [] : priceSizes(legal, math.pot, bigBlind, street),
    });
    plan = gto.plan;
  }
  if (state.phase === 'betting') {
    ...
    sections.splice(2, 0, {
      id: 'plan',
      icon: gto ? '\u265f' : '\u{1F3AF}',
      title: gto ? 'GTO \u5efa\u8bae\uff08\u6700\u5927 EV \u7ebf\uff09' : '\u4e0b\u6ce8\u903b\u8f91\u4e0e\u5efa\u8bae',
      tone: plan.tone,
      lines: [plan.headline, ...plan.reasons],
      items: plan.risks.length > 0 ? [{ label: '风险 / 反面', text: plan.risks.join('；') }] : [],
      tags: plan.tags,
    });
    // The GTO arithmetic sits directly under the line it justifies, before the reading
    // and the review: it IS the reasoning for the recommendation above it.
    if (gto) sections.splice(3, 0, ...gto.sections);
  } else {
  ...
  return {
    version: 1,
    // Which coach produced this: the panel labels the window with it, and the model
    // reads it in the brief.
    mode: gto ? 'gto' : 'simple',
    sections,
    brief,
    tags: plan.tags,
    ...
    plan: { headline: plan.headline, action: plan.action, tone: plan.tone, role: plan.role ?? null },
    equity: vsReads,
    math: { potOdds: math.potOdds, requiredEquity: math.requiredEquity, mdf: math.mdf, spr: math.spr },
    // The quantitative half of the GTO layer, for surfaces that want the numbers
    // without parsing the sections.
    gto: gto ? gto.numbers : null,
  };
}

/**
 * What the opponents have actually been doing when they face a bet.
 *
 * This is the one input the EV of a bet needs that the state cannot infer: how often a
 * bet takes the pot down. The engine keeps that tally per seat (`observed`), so the
 * estimate is the table's real behaviour rather than an assumption - and it falls back
 * to the indifferent frequency when nobody has enough history yet.
 * @param state - game state.
 * @param seat - the hero's seat.
 * @returns a fold frequency in 0.05..0.95.
 */
export function observedFoldRate(state, seat) {
  // Live opponents first: they are the ones who have to fold THIS bet. The rest of the
  // table is the fallback while there is not enough history in the hand.
  const live = state.players.filter((player) => player.seat !== seat && !player.out && !player.folded);
  const others = state.players.filter((player) => player.seat !== seat && !player.out);
  const tallyOf = (players) => players.reduce((acc, player) => {
    const tally = player.observed;
    if (!tally) return acc;
    return {
      faced: acc.faced + (tally.facedBetPost ?? 0),
      folded: acc.folded + (tally.foldedToBetPost ?? 0),
      facedAny: acc.facedAny + (tally.facedBet ?? 0),
      foldedAny: acc.foldedAny + (tally.foldedToBet ?? 0),
    };
  }, { faced: 0, folded: 0, facedAny: 0, foldedAny: 0 });
  const inHand = tallyOf(live);
  if (inHand.faced >= 4) return Math.max(0.05, Math.min(0.95, inHand.folded / inHand.faced));
  const table = tallyOf(others);
  if (table.faced >= 6) return Math.max(0.05, Math.min(0.95, table.folded / table.faced));
  if (table.facedAny >= 8) return Math.max(0.05, Math.min(0.95, table.foldedAny / table.facedAny));
  return 0.45;
}

/**
 * How often a bet takes the pot down RIGHT NOW.
 *
 * The tally gives a PER-OPPONENT fold rate; a bet takes the pot down only when every
 * live opponent folds, so the per-player number is raised to the number of opponents.
 * Using it directly made a two-way flop look like a 60% chance of winning the pot
 * uncontested, which made every bluff look free.
 * @param rate - one opponent's fold-to-bet rate.
 * @param opponents - how many opponents can still fold.
 * @returns a probability in 0.05..0.95.
 */
export function takeDownProbability(rate, opponents) {
  const count = Math.max(1, Number(opponents) || 1);
  const perPlayer = Math.max(0.02, Math.min(0.95, Number(rate) || 0.45));
  return Math.max(0.05, Math.min(0.95, perPlayer ** count));
}

...
import { AI_FIELDS, askAi, normaliseSettings, readSettings, registerSettings, writeSettings } from './ai.js';

// The entry file's browser route answers with the model-facing text as well, so
// the renderer is part of this module's public surface too, and the AI settings
// surface travels with it (the route mounts both).
export { renderTable, AI_FIELDS, askAi, normaliseSettings, readSettings, registerSettings, writeSettings };
...
export function opNew(args, options = {}) {
  ...
  const aiSeats = Math.max(0, Math.min(botCount, Math.floor(options.aiSeats ?? 0)));

...
import { askAi, testConnection } from './ai.js';
import { applyUpdate, checkForUpdate, compareVersions, describeUpdate, fetchLatestRelease, gitArgv, installArgv, parseVersion, pickPackageManager } from './update.js';
import {
...
// The entry file's browser route answers with the model-facing text as well, so the
// renderer is part of this module's public surface too; the settings schema and the
// AI client travel with it (the route mounts all three).
export {
  renderTable,
  askAi,
  testConnection,
...
export function buildView(state, options = {}) {
  const view = {
    ...
    // Which seats the AI endpoint plays, so the panel can label them.
    aiSeats: Array.isArray(state.aiSeats) ? state.aiSeats.slice() : [],
  };
  if (options.coach !== false && state.coachEnabled !== false) {
    const mode = options.coachMode === 'gto' || state.coachMode === 'gto' ? 'gto' : 'simple';
    const coach = coachFor(state, HERO_SEAT, { mode });
    if (coach) {
      // The style the HOST is configured for - which is not the same as the style this
      // payload used: GTO is a postflop layer, so a preflop payload comes from the simple
      // plan even in GTO mode. A surface that switches the style needs the setting (to
      // highlight it, and to notice a host that has not picked the change up yet).
      view.coach = { ...coach, settingsMode: mode };
    }
  }
 ...
 * @param options.defaults - the plugin's settings, used for anything the caller did
 *   not pass (seat count, blinds, starting stack, opponent styles). An argument always
 *   wins: the settings describe the player's *default* table, not a cage.
 * @returns the dealt state, carrying a fresh opaque `tableId`.
 */
export function opNew(args, options = {}) {
  const defaults = options.defaults ?? {};
  const botCount = Math.max(1, Math.min(MAX_BOTS, Math.floor(args.botCount ?? defaults.botCount ?? DEFAULT_BOTS)));
  const rawStyles = String(args.botStyles ?? defaults.botStyles ?? 'mixed').toLowerCase();
  ...
  const state = createGame({
    ...
    bots,
    startingStack: Math.max(200, Math.floor(args.startingStack ?? defaults.startingStack ?? 10000)),
    smallBlind: Math.max(1, Math.floor(args.smallBlind ?? defaults.smallBlind ?? 50)),
    bigBlind: Math.max(2, Math.floor(args.bigBlind ?? defaults.bigBlind ?? 100)),
    botBrain: args.botBrain === 'model' ? 'model' : 'auto',
    seed,
  });
  state.tableId = newTableId();
  state.revision = 0;
  // Seats 1..n are the opponents; the first `aiSeats` of them are played by the
  // configured AI endpoint instead of the heuristic policy.
  const wanted = options.aiSeats ?? (defaults.aiEnabled === true ? defaults.aiSeats : 0);
  const aiSeats = Math.max(0, Math.min(botCount, Math.floor(wanted)));
  state.aiSeats = Array.from({ length: aiSeats }, (_, index) => index + 1);
...
/**
 * Resume a table that paused for an AI seat.
 *
 * The engine stops when the seat on turn is an AI seat; the host asks the endpoint,
 * then hands the answer back through here. A null or unusable answer is not an error:
 * the seat plays the heuristic policy, so an offline endpoint costs nothing but the
 * timeout.
 * @param table - the paused table state.
 * @param decision - `{ action, amount?, talk? }` from the AI, or null.
 * @returns the next state.
 */
export function opResumeAi(table, decision) {
  if (!table || !table.pendingDecision || table.pendingDecision.kind !== 'ai') {
    throw new Error('\u5f53\u524d\u6ca1\u6709\u7b49\u5f85 AI \u51b3\u7b56\u7684\u673a\u5668\u4eba');
  }
  const seat = table.pendingDecision.seat;
  const fallback = decideBotAction(table, seat);
  const chosen = decision && typeof decision === 'object' ? decision : fallback;
  const applied = { seat, action: chosen.action, amount: chosen.amount, talk: chosen.talk };
  return advanceTable(submitBotDecision(table, applied, decideBotAction));
}
